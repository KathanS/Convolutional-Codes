function estimatedMessage = hardDecoder_Intermediate_1_2(receivedCode)   % decoder - temporary name
    ...
    while i<l
        ...
        while k <= length(leaf)
           temp(j) = leaf(k).pathMetric + hammingDistance(twoBits,pairityGenerator_Intermediate_1_2(leaf(k).state,0),flag) + flag;
           temp(j+1) = leaf(k).pathMetric + hammingDistance(twoBits,pairityGenerator_Intermediate_1_2(leaf(k).state,1),flag) + flag;
           j=j+2;
           k=k+1;
        end
