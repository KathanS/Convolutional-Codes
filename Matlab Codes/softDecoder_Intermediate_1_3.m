function estimatedMessage = softDecoder_Intermediate_1_3(receivedCode)   % decoder - temporary name
    ...
    while i<l
        threeVolts = [receivedCode(i),receivedCode(i+1),receivedCode(i+2)];
        temp = zeros(1,2*length(leaf))-1;
        j=1;
        k=1;
        while k <= length(leaf)
           temp(j) = leaf(k).pathMetric + euclidieanDistance(threeVolts,pairityGenerator_Intermediate_1_3(leaf(k).state,0));
           temp(j+1) = leaf(k).pathMetric + euclidieanDistance(threeVolts,pairityGenerator_Intermediate_1_3(leaf(k).state,1));
           j=j+2;
           k=k+1;
        end
