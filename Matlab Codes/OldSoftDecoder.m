function estimatedMessage = OldSoftDecoder(receivedCode)   % decoder - temporary name
    ...
    while i<l
        ...
        for x = 1:length(minIndex)
            ...
            p = length(tree)-length(leaf)+ceil(minIndex(x)/2);
            newLeaf(x).parent = tree(p).location;
            tree(p).child = [tree(p).child,newLeaf(x).location];
            newLeaf(x).state = stateGenerator(tree(p).state,mod(minIndex(x)+1,2));
