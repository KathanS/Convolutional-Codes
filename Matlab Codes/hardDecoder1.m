function estimatedMessage = hardDecoder1(receivedCode)   % decoder - temporary name
    ...
    parents(3) = 0;
    parents(4) = 0;
    states = zeros(1,((length(receivedCode)-4)/2+1)*4);
    states(1) = 0;
    states(2) = 1;
    states(3) = 2;
    states(4) = 3;
    ...
    while i<l
        ...
        while k <= 4
           temp(j) = pathMetric(4*s+k) + hammingDistance(twoBits,pairityGenerator(states(4*s+k),0),flag) + flag;
           temp(j+1) = pathMetric(4*s+k) + hammingDistance(twoBits,pairityGenerator(states(4*s+k),1),flag) + flag;
           j=j+2;
           k=k+1;
        end
