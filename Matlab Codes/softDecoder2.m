function estimatedMessage = softDecoder2(receivedCode)   % decoder - temporary name
    ...
    while i<l
        ...
        k=1;
        while k <= 4
           temp(j) = pathMetric(4*s+k) + euclidieanDistance(twoVolts,2*pairityGenerator(k-1,0)-1);
           temp(j+1) = pathMetric(4*s+k) + euclidieanDistance(twoVolts,2*pairityGenerator(k-1,1)-1);
           j=j+2;
           k=k+1;
        end
