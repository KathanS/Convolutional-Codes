function estimatedMessage = softDecoder_Intermediate_1_2(receivedCode)   % decoder - temporary name
    ...
    while i<l
        ...
        k=1;
        while k <= length(leaf)
           temp(j) = leaf(k).pathMetric + euclidieanDistance(twoVolts,pairityGenerator_Intermediate_1_2(leaf(k).state,0));
           temp(j+1) = leaf(k).pathMetric + euclidieanDistance(twoVolts,pairityGenerator_Intermediate_1_2(leaf(k).state,1));
           j=j+2;
           k=k+1;
        end
