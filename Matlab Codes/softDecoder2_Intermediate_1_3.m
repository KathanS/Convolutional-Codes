function estimatedMessage = softDecoder2_Intermediate_1_3(receivedCode)   % decoder - temporary name
    ...
    while i<l
        ...
        k=1;
        while k <= 8
           temp(j) = pathMetric(8*s+k) + euclidieanDistance(threeVolts,2*pairityGenerator_Intermediate_1_3(k-1,0)-1);
           temp(j+1) = pathMetric(8*s+k) + euclidieanDistance(threeVolts,2*pairityGenerator_Intermediate_1_3(k-1,1)-1);
           j=j+2;
           k=k+1;
        end
